...
import { CONTATO } from './site/siteConfig'
...
import { EmBreveProvider } from './site/EmBreve'
import RegistroEmBreve from './site/RegistroEmBreve'
...
const ORG_JSON_LD = {
  ...
  contactPoint: [{
    '@type': 'ContactPoint',
    telephone: `+${CONTATO.whatsappDigits}`,
...
// Área pós-login do cliente. Estas rotas continuam sendo da plataforma (o
// conteúdo é o mesmo de sempre), mas a barra de cima passa a ser a do site —
// mesma marca, mesmos links, mesmos botões. A barra antiga tinha a faixa verde
// no topo e um mega-menu com endereços que já não existem.
// Mantida em sincronia com CLIENT_ROUTE_PREFIXES em components/ClientNav.jsx.
const CLIENT_AREA_PREFIXES = ['/perfil', '/cotacao', '/meus-pedidos', '/catalogo-legado']

const isClientAreaRoute = (pathname) =>
  CLIENT_AREA_PREFIXES.some((p) => pathname === p || pathname.startsWith(p + '/'))
...
function Shell() {
  ...
  const isClientArea = isClientAreaRoute(pathname)

  return (
    <>
      <Seo jsonLd={ORG_JSON_LD} />
      <ScrollToTop />
      {!hideChrome && (isClientArea ? <PlatformNav /> : <Navbar />)}
        ...
        <RouteErrorBoundary>
          <Suspense fallback={<RouteLoader />}>
            <Routes>
              ...
              {/* O cadastro público fica atrás de um porteiro enquanto a plataforma
                  está em construção (PLATAFORMA.bloquearCadastroPublico). A página
                  de login continua aberta: a equipe e o admin precisam entrar. */}
              <Route path="/registro" element={<RegistroEmBreve><Registro /></RegistroEmBreve>} />
