...
export default function MaquinaNova() {
  ...
  const validateStep = (s) => {
    ...
    if (s === 2) {
      if (!form.photo) return 'A foto principal da máquina é obrigatória'
    }
    // Step 3 é opcional (conjuntos + manual)
    if (s === 3) {
      if (selectedCustom && !form.customAssemblyName.trim()) {
        return `Especifique o nome do conjunto para "${selectedCustom.name}"`
      }
    }
    return null
  }
    ...
    <section className="dashboard">
      ...
      <div className="container" style={{ paddingBottom: 80 }}>
        ...
        <div className="machine-wizard">
            ...
            <div className="machine-wizard-panel">
              <div className="machine-wizard-panel-head">
                ...
                <div className="machine-wizard-panel-title">
                  ...
                  <p>Uma foto principal (obrigatória) e até várias fotos extras (opcional).</p>
                </div>
              </div>

              <label style={{ display: 'block', marginBottom: 12, fontSize: '0.92rem', fontWeight: 600 }}>
                Foto principal <span style={{ color: 'var(--danger)' }}>*</span>
              </label>
              <div
              ...
              >
                {form.photoPreview ? (
                  ...
                  <div className="photo-drop-placeholder">
                    ...
                    <strong>Clique para enviar a foto principal</strong>
          ...
          {/* STEP 3: DETALHES (TODOS OPCIONAIS) */}
          {step === 3 && (
            <div className="machine-wizard-panel">
              <div className="machine-wizard-panel-head">
                <div className="machine-wizard-panel-icon"><Layers size={20} /></div>
                <div className="machine-wizard-panel-title">
                  <h2>Detalhes técnicos</h2>
                  <p>Tudo opcional. Ajuda a personalizar o catálogo, mas você pode adicionar depois.</p>
                </div>
              </div>

              <label style={{ display: 'block', marginBottom: 12, fontSize: '0.92rem', fontWeight: 600 }}>
                Conjuntos mecânicos de interesse <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '0.86rem' }}>(opcional)</span>
              </label>
              <div className="select-grid">
                {assemblies.map(a => (
                  <button
                    key={a.id}
                    type="button"
                    className={`select-card ${form.assemblyIds.includes(a.id) ? 'active' : ''}`}
                    onClick={() => toggleAssembly(a.id)}
                  >
                    <div className="select-card-title">
                      <Cog size={14} /> {a.name}
                    </div>
              ...
              {selectedCustom && (
                <div className="form-group" style={{ marginTop: 20 }}>
                  <label>Nome customizado para "{selectedCustom.name}"</label>
                  <input
                    type="text"
                    className="form-input"
                    placeholder="Descreva o conjunto"
                    value={form.customAssemblyName}
                    onChange={(e) => setForm({ ...form, customAssemblyName: e.target.value })}
                  />
                </div>
              )}
          ...
          {step === 4 && (
            <div className="machine-wizard-panel">
              ...
              <div className="wizard-review">
                ...
                <div className="wizard-review-item">
                  <div className="wizard-review-item-label">Conjuntos selecionados</div>
                  <div className="wizard-review-item-value">
                    {form.assemblyIds.length === 0
                      ? <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>Nenhum (opcional)</span>
                      : `${form.assemblyIds.length} conjunto${form.assemblyIds.length > 1 ? 's' : ''}`}
                  </div>
                </div>
                <div className="wizard-review-item span-2">
                  <div className="wizard-review-item-label">Fotos enviadas</div>
                  <div className="wizard-review-item-value">
                    1 principal{form.extraPhotos.length > 0 && ` + ${form.extraPhotos.length} extra${form.extraPhotos.length > 1 ? 's' : ''}`}
              ...
              {form.photoPreview && (
                <div style={{
                  ...
                  <img src={form.photoPreview} alt="Foto principal" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              )}
