...
/* v80: a primeira seção de Sobre virou hero de vídeo, igual à da Home —
   então ela entra no grupo de hero escuro (nav branca até rolar 40px). */
const DARK_HERO: Record<SitePage, boolean> = {
  home: true, produtos: true, servicos: true, pecas: false, sobre: true,
  ...
  return (
    ...
    <nav id="nav" className={scrolled ? 'scrolled' : undefined}>
      <div className="nav_in">
        ...
        {/* Os mesmos quatro botões de sempre, no mesmo lugar e com o mesmo
            texto. A única diferença: enquanto a plataforma do cliente está em
            construção (PLATAFORMA.ativo em site/siteConfig.ts), o clique abre
            o aviso com a centrifuguinha em vez de navegar. Virou `false`, eles
            voltam a navegar sozinhos. */}
        <div className="nav_actions">
          {isAuthenticated ? (
            <>
              <BtnPlataforma to={R.perfil} className="btn btn_enter">
                Minha conta
              </BtnPlataforma>
              <BtnPlataforma to={R.catalogo} className="btn btn_solid">
                Catálogo
              </BtnPlataforma>
            </>
          ) : (
            <>
              <BtnPlataforma to={R.login} className="btn btn_enter">
                Entrar
              </BtnPlataforma>
              <BtnPlataforma to={R.registro} className="btn btn_solid">
                Cadastrar
              </BtnPlataforma>
            </>
          )}
      ...
      >
        ...
        <div className="nav_drawer_actions" onClick={() => setMenuOpen(false)}>
          {isAuthenticated ? (
            <>
              <BtnPlataforma to={R.perfil} className="btn btn_enter">
                Minha conta
